...
for k = 1:tam(1)
    ...
    phi_ref = atan2(y_ref(k) - pos(2), x_ref(k) - pos(1)) + randn() * vari; % Agregar ruido a la referencia
    ...
    % Modelo robot móvil con ruido
    d_pos = [vr * cos(pos(3)); vr * sin(pos(3)); vr / d * tan(alpha)]; % [x';y';phi']
    ruido = randn(3, 1)*vari; % ruido con media cero y desviación estándar 1
    pos = pos + Ts * d_pos + ruido; % Integración euler con ruido
