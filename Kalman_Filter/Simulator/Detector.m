function Caracteristicas = Detector(Laser,robot)
global maxRange;
Cart = [];
Postes = [];
for j=1:length(Laser)
    if(Laser(1,j)<0.9*maxRange)
        Cart = [Cart;Laser(1,j)*cos(Laser(2,j)*pi/180-pi/2) Laser(1,j)*sin(Laser(2,j)*pi/180-pi/2) j];
    end
end 
if(~isempty(Cart))
    for cont=1:length(Cart(:,1))
        ...
        Postes = [Postes; xx yy Cart(cont,3)];
    end
end
[f,c] = size(Postes);
